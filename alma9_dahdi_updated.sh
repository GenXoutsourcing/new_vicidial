...
# vmlinux link for kernel build/debug symbols, non-fatal if missing
...
# clean DAHDI source folder every run
cd /usr/src/ || exit 1
...
# AlmaLinux 9 / newer 5.14.0-687.x kernel DAHDI fixes
# Fix core sysfs callback signature change: struct device_driver * -> const struct device_driver *
...
# Disable old/unused physical hardware drivers that fail on newer EL9 kernels.
# VICIdial SIP/WebRTC installs only need DAHDI core timing, not these T1/E1/FXO/FXS/Astribank drivers.
sed -i \
  -e '/wctc4xxp/d' \
  -e '/wcte12xp/d' \
  -e '/wcte13xp/d' \
...
# Build/install DAHDI tools from this same source package.
# Do NOT install dahdi-tools-libs from yum here; it can cause old tools/library mismatch.
cd tools || exit 1
make clean
make
make install
...
# Fix libtonezone.so.2 lookup after source install
echo "/usr/local/lib" > /etc/ld.so.conf.d/dahdi-local.conf
echo "/usr/local/lib64" >> /etc/ld.so.conf.d/dahdi-local.conf
ldconfig
...
# dahdi_test runs forever by default, so limit it and continue the install
timeout 10s dahdi_test 2>&1 | tee /tmp/dahdi_test.log || true
echo "DAHDI install complete, continuing script..."
